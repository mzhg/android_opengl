...
uniform float g_OutterRadus;
uniform float g_InnerRadius;
uniform vec3  g_RingColor;
uniform vec2  g_Center;  // the center of the screen.
uniform float g_ElpsedTime;
uniform float g_TimeThreshold;

#define PI 3.1415926

void main() {
    vec2 dirToScreenPos = gl_FragCoord.xy - g_Center;
    float length = length(dirToScreenPos);
    if(length < g_InnerRadius || length > g_OutterRadus)
    {
        discard;
    }

    if(g_ElpsedTime > g_TimeThreshold)
    {
        gl_FragColor = vec4(g_RingColor, 1);
    }
    else
    {
        ...
        float angleThreshold = g_ElpsedTime / g_TimeThreshold * 2.0 * PI;
