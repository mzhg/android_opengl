...
//	private HDRImage[] image = new HDRImage[4];
    ...
    private NvSampleApp mApp;
    private AntvrSDK mAntvrSDK;//for gyro data

    public HDRScene(NvSampleApp app){
        mApp = app;
        mAntvrSDK = AntvrSDK.getInstance(mApp);
    }

    public void startHeadTracking() { mAntvrSDK.onStart();}
    public void stopHeadTracking()  { mAntvrSDK.onStop();}

    public void initUI() {
        @SuppressWarnings("unused")
        NvTweakVarBase var;

        NvTweakBar tweakBar = mApp.getTweakBar();
        var = tweakBar.addValue("Auto Spin", NvSampleApp.createControl(this,"m_autoSpin"));
//        var = tweakBar.addValue("Draw Background", createControl("m_drawBackground"));
        var = tweakBar.addValue("HDR Post", NvSampleApp.createControl(this,"m_hdr_post"));
...
//        tweakBar.addPadding();
        ...
        tweakBar.addMenu("Select Scene:", NvSampleApp.createControl(this,"m_sceneIndex"), sceneIndex, 0x22);
        ...
        tweakBar.addMenu("Select Material:", NvSampleApp.createControl(this,"m_materialIndex"), materialIndex, 0x33);
        ...
        tweakBar.addEnum("Select Object:", NvSampleApp.createControl(this,"m_objectIndex"), objectIndex, 0x55);
        ...
        tweakBar.addEnum("Glare Type:", NvSampleApp.createControl(hdr_params, "glareType"), glareType, 0x44);
        tweakBar.syncValues();
    }
...
/*
        ...
        */
        ...
        mApp.getInputTransformer().setTranslationVec(new Vector3f(0.0f, 0.0f, -202.2f));
        mApp.getInputTransformer().setRotationVec(new Vector3f(-0.2f, -0.3f, 0));
    }

    public void onDraw() {
        FrameBufferObject source = scene_buffer;
        mApp.getInputTransformer().setRotationVel(new Vector3f(0.0f, m_autoSpin ? (NvUtils.PI *0.05f) : 0.0f, 0.0f));
        ...
        */
        mApp.getInputTransformer().getModelViewMat(temp);
        float[] rotateMat = mAntvrSDK.getRotationMatrix();
        rotation.load(rotateMat, 0);
        Matrix4f.mul(temp, rotation, temp);
...
//	    	GL20.glVertexAttribPointer(2, 2, GL11.GL_FLOAT, false, 32, 24);
        ...
        if(m_hdr_post){
            hdr_params.elpsedTime = mApp.getFrameDeltaTime();

            hdr_params.scene_texture = scene_buffer.getColorTexture(0);
            hdr_params.scene_width = mApp.getWidth();
            hdr_params.scene_height = mApp.getHeight();
...
//            hdr_params.blendAmount = 1;
            ...
            GLES30.glViewport(0, 0, mApp.getWidth(), mApp.getHeight());
