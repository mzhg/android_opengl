...
//					v0.store(tempArray, 3 * index);
		...
		}else if(builder.flag == false){
			model = new Model(builder.flag);
			model.addAttrib(positions, Model.TYPE_VERTEX);
			if(normals != null)
				model.addAttrib(normals, Model.TYPE_NORMAL);
			if(texCoords != null)
				model.addAttrib(texCoords, Model.TYPE_TEXTURE0);
			if(colors != null)
				model.addAttrib(colors, Model.TYPE_COLOR);
		}
//		else{  TODO Never happend
//			throw new IllegalArgumentException("Invalid flag: " + builder.flag);
//		}
