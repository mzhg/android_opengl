...
//					index[info.index] = true;
	...
	public GLVAO genVAO(boolean programed){
		valid();
		
		GLVAO vao = new GLVAO(this, programed);
		vaos.add(vao);
		return vao;
	}
	...
	public void setElements(AttribArray element){
		...
		if(/*LWJGLUtil.CHECKS &&*/ element != null){
			...
			if(type != GL11.GL_BYTE  && type != GL11.GL_UNSIGNED_BYTE &&
			   type != GLES30.GL_INT   && type != GLES30.GL_UNSIGNED_INT &&
			   type != GL11.GL_SHORT && type != GL11.GL_UNSIGNED_SHORT)
