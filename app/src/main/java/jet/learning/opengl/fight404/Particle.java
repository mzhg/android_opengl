...
final class Particle {
    ...
    Particle reset(Vector3f _loc, Vector2f _vel){
        gen = 1;
        radius = NvUtils.random(10 - gen, 50 - (gen - 1) * 10);
        loc.set(_loc);
        loc.x += NvUtils.random(0, 1);
        loc.y += NvUtils.random(0, 1);
        loc.z += NvUtils.random(0, 1);
//			vel.set(_vel.x, _vel.y);
        float angle = NvUtils.random(0, NvUtils.PI * 2.0f);
        vel.set(0,0,0);
        vel.z += Math.cos(angle);
        vel.x += Math.sin(angle);
        vel.y += -Math.sqrt(1.0 - vel.x * vel.x - vel.y * vel.y);
        vel.scale(NvUtils.random(10, 20));
        age = 0;
        bounceAge = 2;
