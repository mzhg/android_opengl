...
public class HDRImage {
	...
	protected int _format = GL11.GL_RGBA;
	protected int _internalFormat = GL11.GL_RGBA;
	protected int _type = GL11.GL_UNSIGNED_BYTE;
	...
	public float[] getLevel( int level, int face){
	    ...
	    assert( _faces == 0 || ( face >= GLES20.GL_TEXTURE_CUBE_MAP_POSITIVE_X && face <= GLES20.GL_TEXTURE_CUBE_MAP_NEGATIVE_Z));
	...
	public boolean loadHDRIFromFile(String file){
		...
		try {
			BufferedInputStream in = new BufferedInputStream(NvAssetLoader.openInputStream(file));
...
//			NvLogger.e("RGBE error: %s\n", msg);
	...
	static void float2rgbe(byte[] rgbe, float red, float green, float blue) {
		...
		if (v < 1e-32) {
			...
			e = NvUtils.getExponent(v) + 1;
			v = v / NvUtils.ldexp(1.0f, e) * 256.0f / v;
			rgbe[0] = (byte) (red * v);
			rgbe[1] = (byte) (green * v);
			rgbe[2] = (byte) (blue * v);
			rgbe[3] = (byte) (e + 128);
	...
	static void rgbe2float(float[] rgb, int offset, byte[] rgbe) {
		...
		if (rgbe[3] != 0) { /* nonzero pixel */
			f = (float) NvUtils.ldexp(1.0, (rgbe[3] & 0xFF) - (int) (128 + 8));
