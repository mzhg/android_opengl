package com.antvr.antvrsdk.sensor.model;
...
public class OrientationEKF {
    ...
    public float[] rotationMatrix_ = new float[9];
    public float[] orientation = new float[3];
    
    public synchronized void setAccel(float[] sensorValues){
    	System.arraycopy(sensorValues, 0, accel, 0, 3);
    }
    
    public synchronized void setMag(float[] sensorValues){
    	System.arraycopy(sensorValues, 0, mag, 0, 3);
    }
    
    public synchronized float[] calculateMagAccOrientation(){
    	if(SensorManager.getRotationMatrix(rotationMatrix_, null, accel, mag)){
    		SensorManager.getOrientation(rotationMatrix_, orientation);
    	}
    	return orientation;
    }
