 ...
 * @see NvBitFont
 */
public class NvBFText implements NvUITextAlign, NvDisposeable{
...
//	static float s_bfShadowMultiplier = 0.40f;
	...
	private static final int[] s_charColorTable = {
		NvUtils.makefourcc(0xFF, 0xFF, 0xFF, 0xFF), //white
		NvUtils.makefourcc(0x99, 0x99, 0x99, 0xFF), //medium-gray
		NvUtils.makefourcc(0x00, 0x00, 0x00, 0xFF), //black
		NvUtils.makefourcc(0xFF, 0x33, 0x33, 0xFF), //brightened red
		NvUtils.makefourcc(0x11, 0xFF, 0x11, 0xFF), //brighter green
		NvUtils.makefourcc(0x33, 0x33, 0xFF, 0xFF) //brightened blue
	};
    ...
    protected int m_shadowColor = NvPackedColor.NV_PC_PREDEF_BLACK;
...
//            free(m_data);
    ...
    /**
     * Set the RGBA color (using an {@link NvPackedColor}) for a line of text.<p>
    ...
    /**
     * Set the RGBA color (an {@link NvPackedColor}) for a line of text.  As this color is multiplied
    ...
    public void setString(String str){
    	if (NvUtils.isEmpty(str))
        {
            NvLogger.i("bitfont sent a null string ptr...");
//        if (m_string==NULL)
...
//                free(masterTextIndexList);
            ...
            if (!internalCall)
                GLES.glBindBuffer(GL11.GL_ELEMENT_ARRAY_BUFFER, NvBitFont.masterTextIndexVBO);
            GLES.glBufferData(GL11.GL_ELEMENT_ARRAY_BUFFER, GLUtil.wrap(masterTextIndexList), GL11.GL_STATIC_DRAW);
            if (!internalCall)
            	GLES.glBindBuffer(GL11.GL_ELEMENT_ARRAY_BUFFER, 0); // !!!!TBD resetting here, if we're INSIDE the render call, is wasteful... !!!!TBD
        }
    ...
    private void adjustGlyphsForAlignment(){
        ...
        NvLogger.i("Adjusting glyphs for alignment...");
        ...
        for (int i=0; i<m_numLines; i++)
        {
            ...
            NvLogger.i("line %d, char count %d.", i, max);
...
//            free(m_calcLineWidth);
     ...
     * {@link #setCursorAlign}, in possible combination with {@link setSetBox}.
     * That is, the borders to offset from are "screen edges", unless {@link setSetBox}
...
//        m_hModeVBO = -1;
    ...
    public void renderPrep(){
    	...
    	NvGLSLProgram fontProg = NvBitFont.fontProg;
        fontProg.enable();
...
//        TestPrintGLError("Error 0x%x in NvBFText::RenderPrep...\n");
    ...
    public void renderDone(){
        ...
        else
        {
            ...
            GLES20.glDisableVertexAttribArray(NvBitFont.fontProgAttribPos);
            GLES20.glDisableVertexAttribArray(NvBitFont.fontProgAttribTex);
            GLES20.glDisableVertexAttribArray(NvBitFont.fontProgAttribCol);
    ...
    public void setMatrix(float[] mat){
    	...
    	if(mat != null)
    		GLES20.glUniformMatrix4fv(NvBitFont.fontProgLocMat, 1, false, GLUtil.wrap(mat));
    }
    ...
    public void render(){
        ...
        // set up master rendering state
        {
            int offset = 0;
            if (m_vbo == 0)
            	m_vbo = GLES.glGenBuffers(); // !!!!TBD TODO error handling.
            GLES.glBindBuffer(GL11.GL_ARRAY_BUFFER, m_vbo);

            GLES20.glVertexAttribPointer(NvBitFont.fontProgAttribPos, 2, GL11.GL_FLOAT, false, 20, 0);
            GLES20.glEnableVertexAttribArray(NvBitFont.fontProgAttribPos);
            offset += 4 * 2; // jump ahead the two floats

            GLES20.glVertexAttribPointer(NvBitFont.fontProgAttribTex, 2, GL11.GL_FLOAT, false, 20, offset); // !!!!TBD update this to use a var if we do 2 or 3 pos verts...
            GLES20.glEnableVertexAttribArray(NvBitFont.fontProgAttribTex);
            offset += 4 * 2; // jump ahead the two floats.

            GLES20.glVertexAttribPointer(NvBitFont.fontProgAttribCol, 4, GL11.GL_UNSIGNED_BYTE, true, 20, offset); // !!!!TBD update this to use a var if we do 2 or 3 pos verts...
            GLES20.glEnableVertexAttribArray(NvBitFont.fontProgAttribCol);
            offset += 4;
        }
        ...
        if (NvBitFont.m_matrixOverride==null)
        {
            ...
            GLES20.glUniformMatrix4fv(NvBitFont.fontProgLocMat, 1, false, GLUtil.wrap(s_pixelToClipMatrix));
        }

        // bind texture... now with simplistic state caching
        if (NvBitFont.lastFontTexture != m_font.m_tex)
        {
        	GLES.glBindTexture( GL11.GL_TEXTURE_2D, m_font.m_tex );
        ...
        GLES.glDrawElements(GL11.GL_TRIANGLES, IND_PER_QUAD * count, GL11.GL_UNSIGNED_SHORT, 0);
...
//        TestPrintGLError("Error 0x%x NvBFText::Render drawels...\n");
    ...
    public void rebuildCache(boolean internal){
        ...
        int n,j;
        int vp, lastvp;
        int vbmode = GL11.GL_STATIC_DRAW /*GL_DYNAMIC_DRAW*/;
...
//                extrawrapmargin = truncit->second.m_xAdvance;
        ...
        GLES.glBufferData(GL11.GL_ARRAY_BUFFER, /*m_stringCharsOut*sizeof(BFVert)*VERT_PER_QUAD,*/ wrap(m_data, m_stringCharsOut * VERT_PER_QUAD), vbmode);
        if (!internal)
        	GLES.glBindBuffer(GL11.GL_ARRAY_BUFFER, 0); // !!!!TBD resetting here, if we're INSIDE the render call, is wasteful... !!!!TBD
...
//        TestPrintGLError("Error 0x%x in SaveState @ start...\n");
        ...
        gStateBlock.programBound = GLES.glGetInteger(GLES20.GL_CURRENT_PROGRAM);
...
//            	gStateBlock.attrib[i].ptr = GL20.glGetVertexAttribPointer(i, GL20.GL_VERTEX_ATTRIB_ARRAY_POINTER, gStateBlock.attrib[i].ptr_size);
        ...
        gStateBlock.depthMaskEnabled = GLES.glGetBoolean(GL11.GL_DEPTH_WRITEMASK);
        gStateBlock.depthTestEnabled = GLES.glIsEnabled(GL11.GL_DEPTH_TEST);
        gStateBlock.blendEnabled = GLES.glIsEnabled(GL11.GL_BLEND);
        gStateBlock.cullFaceEnabled = GLES.glIsEnabled(GL11.GL_CULL_FACE);
//        glGetIntegerv(GL_CULL_FACE_MODE, &(gStateBlock.cullMode));

        gStateBlock.vboBound = GLES.glGetInteger(GL11.GL_ARRAY_BUFFER_BINDING);
        gStateBlock.iboBound = GLES.glGetInteger(GL11.GL_ELEMENT_ARRAY_BUFFER_BINDING);
        gStateBlock.texBound = GLES.glGetInteger(GL11.GL_TEXTURE_BINDING_2D);
        gStateBlock.texActive = GLES.glGetInteger(GL11.GL_ACTIVE_TEXTURE);
...
//        TestPrintGLError("Error 0x%x in RestoreState @ start...\n");
        ...
        GLES20.glUseProgram(gStateBlock.programBound);

        // set buffers first, in case attribs bound to them...
        GLES.glBindBuffer(GL11.GL_ARRAY_BUFFER, gStateBlock.vboBound);
        GLES.glBindBuffer(GL11.GL_ELEMENT_ARRAY_BUFFER, gStateBlock.iboBound);
        ...
        if (gStateBlock.depthMaskEnabled)
        	GLES.glDepthMask(true); // we turned off.
        if (gStateBlock.depthTestEnabled)
        	GLES.glEnable(GL11.GL_DEPTH_TEST); // we turned off.
        if (!gStateBlock.blendEnabled)
        	GLES.glDisable(GL11.GL_BLEND); // we turned ON.
        if (gStateBlock.cullFaceEnabled)
        	GLES.glEnable(GL11.GL_CULL_FACE); // we turned off.
//        glGetIntegerv(GL_CULL_FACE_MODE, &(gStateBlock.cullMode));

        // restore tex BEFORE switching active state...
        GLES.glBindTexture(GL11.GL_TEXTURE_2D, gStateBlock.texBound);
        if (gStateBlock.texActive != GL11.GL_TEXTURE0)
        	GLES.glActiveTexture(gStateBlock.texActive); // we set to 0

//        TestPrintGLError("Error 0x%x in RestoreState @ end...\n");
    }
    
    private static int glGetVertexAttribiv(int index, int pname){
    	return GLES.glGetVertexAttribi(index, pname);
    }
    
    private static ByteBuffer wrap(BFVert[] data, int length){
    	ByteBuffer buffer = GLUtil.getCachedByteBuffer(4 * 5 * length);
