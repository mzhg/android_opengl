...
public class NvTweakBar extends NvUIContainer {
	...
	public NvTweakBar(float width, float height) {
	    ...
	    m_labelColor = NvUtils.makefourcc(64,192,192,255);
	    // keep value white until we can pass through to buttons and others
	    m_valueColor = NvUtils.makefourcc(255,255,255,255); 
...
//	    frame.setAlpha(0.5f);
	    frame.setColor(NvUtils.makefourcc(0x60, 0xFF, 0xFF, 0xFF));
	    return frame;
	}
	...
	private NvUIPopup makeStdPopup(String name, NvTweakEnumVari refvar, NvTweakEnumi[] values, int actionCode /*=0*/){
	    ...
	    frame.setAlpha(1.8f);
	    frame.setDimensions(wide, high);
	    frame.setColor(NvUtils.makefourcc(0xFF, 0xE0, 0x50, 0xFF)); // yellow
	    els[1] = frame;
	...
	*/
	    ...
	    fullbar.setColor(NvUtils.makefourcc(0xFF, 0xE0, 0x50, 0xFF)); // yellow
...
//	    AddElement(vtxt, !m_compactLayout);
	...
	public NvTweakVarBase addValueReadout(NvTweakVarf tvar, float max) {
	    ...
	    vtxt.setValueColor(NvPackedColor.NV_PC_PREDEF_WHITE);
